 ...
 */
class Bew_EDD_Frontend_Submissions {
	...
	public function remove_body_classes() {
		...
		if ( $this->is_single_vendor_page() ) {
			remove_filter( 'body_class', 'bew_body_classes' );
		}
	}
	...
	public function single_vendor_page_download_columns( $out, $pairs, $atts, $shortcode ) {
		...
		if ( $this->is_single_vendor_page() ) {
			$out['columns'] = apply_filters( 'bew_edd_fes_single_vendor_page_columns', 2 );
		}
	...
	public function contact_form_title( $title ) {
		return apply_filters( 'bew_edd_fes_contact_form_title', __( 'Contact vendor', 'briefcase-elementor-widgets' ) );
	}
...
new Bew_EDD_Frontend_Submissions;
