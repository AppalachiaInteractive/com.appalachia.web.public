...
class Updraft_Manager_Updater_1_5 {
	...
	public function admin_menu() {
		...
		if (!empty($this->plug_updatechecker) && !empty($this->plug_updatechecker->optionName) && current_user_can('update_plugins')) {
			...
			|| (('options-general.php' == $pagenow || 'admin.php' == $pagenow) && !empty($_REQUEST['page']) && $this->slug == $_REQUEST['page'])
			|| (('options-general.php' == $pagenow || 'admin.php' == $pagenow) && !empty($_REQUEST['page']) && 'bew-templates' == $_REQUEST['page'])) {
				$options = $this->get_option($this->option_name);
