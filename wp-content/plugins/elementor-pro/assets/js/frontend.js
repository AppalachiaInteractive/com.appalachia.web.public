/*! elementor-pro - v3.2.2 - 11-05-2021 */
...
var VideoPlaylistHandler = /*#__PURE__*/function (_TabsModule) {
  ...
  (0, _createClass2.default)(VideoPlaylistHandler, [{
    ...
    value: function activateInitialVideo() {
      ...
      var isUrlParamsExit = false;

      if (initialTabIndex) {
        this.currentPlaylistItemIndex = initialTabIndex;
        isUrlParamsExit = true;
      } else {
        this.currentPlaylistItemIndex = 1;
      }

      this.handleFirstVideoActivation(isLazyLoad, isUrlParamsExit);
    }
    /*
    ...
    value: function handleFirstVideoActivation(isLazyLoad, isUrlParamsExit) {
      var _this2 = this;

      if (!isLazyLoad && !isUrlParamsExit) {
    ...
    value: function changeActiveTab(tabIndex, isVideoSelectedAutomatically) {
      ...
      if (this.playlistItemsArray[tabIndex - 1] && this.playlistItemsArray[tabIndex - 1].videoType !== 'section') {
        ...
        if (jQuery(this.elements.$innerTabs.eq(tabIndex - 1)).find('.e-inner-tab-content').length > 0) {
          ...
          (0, _innerTabs.handleInnerTabsButtonsDisplay)(innerTabsContent, this.isCollapsible, this.innerTabsHeightLimit);
        }
      }
    }
  }]);
...
function toggleInnerTabs(event, clickedTab, widgetObject) {
  ...
  handleInnerTabsButtonsDisplay(tabsContents); // Trigger event when tab open.
...
function handleInnerTabsButtonsDisplay(tabsContents, isCollapsible, innerTabsHeightLimit) {
  ...
  var activeInnerTab = tabsContents.filter('.e-inner-tab-active'),
      innerTabScrollableHeight = activeInnerTab.find('.e-inner-tab-text')[0].scrollHeight;

  if (innerTabsHeightLimit.size && innerTabScrollableHeight > innerTabsHeightLimit.size) {
    activeInnerTab.addClass('show-inner-tab-buttons');
  }
}
