...
class Admin {
	...
	public static function get_errors_details() {
		...
		return [
			API::STATUS_EXPIRED => [
				...
				'description' => sprintf( esc_html__( '<a href="%s" target="_blank">Renew your license today</a>, to keep getting feature updates, premium support and unlimited access to the template library.', 'elementor-pro' ), API::RENEW_URL ),
			...
			API::STATUS_DISABLED => [
				...
				'description' => esc_html__( '<strong>Your license key has been cancelled</strong> (most likely due to a refund request). Please consider acquiring a new license.', 'elementor-pro' ),
			...
			API::STATUS_INVALID => [
				...
				'description' => esc_html__( '<strong>Your license key doesn\'t match your current domain</strong>. This is most likely due to a change in the domain URL of your site (including HTTPS/SSL migration). Please deactivate the license and then reactivate it again.', 'elementor-pro' ),
			...
			API::STATUS_SITE_INACTIVE => [
				...
				'description' => esc_html__( '<strong>Your license key doesn\'t match your current domain</strong>. This is most likely due to a change in the domain URL. Please deactivate the license and then reactivate it again.', 'elementor-pro' ),
				'button_text' => esc_html__( 'Reactivate License', 'elementor-pro' ),
				'button_url' => $license_page_link,
			],
		];
	...
	public function action_activate_license() {
		...
		if ( is_wp_error( $data ) ) {
			wp_die( sprintf( '%s (%s) ', $data->get_error_message(), $data->get_error_code() ), esc_html__( 'Elementor Pro', 'elementor-pro' ), [
				'back_link' => true,
			] );
		}
		...
		if ( API::STATUS_VALID !== $data['license'] ) {
			...
			wp_die( $error_msg, esc_html__( 'Elementor Pro', 'elementor-pro' ), [
				'back_link' => true,
			] );
		}
			...
			<form class="elementor-license-box" method="post" action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>">
				...
				<?php endif; ?>
					...
					<p class="e-row-stretch e-row-divider-bottom">
						...
						if ( $connected_user ) :
							echo sprintf( esc_html__( 'You\'re connected as %s.', 'elementor-pro' ), '<strong>' . $this->get_connected_account() . '</strong>' );
						endif;
						?>
		...
		if ( API::is_license_active() ) {
			if ( API::is_license_about_to_expire() ) {
				...
				if ( isset( $license_data['renewal_discount'] ) && 0 < $license_data['renewal_discount'] ) {
					$description = sprintf( esc_html__( '<a href="%1$s" target="_blank">Renew your license today</a>, and get an exclusive, time-limited %2$s discount.', 'elementor-pro' ), $renew_url, $license_data['renewal_discount'] . '%' );
				} else {
					$description = sprintf( esc_html__( '<a href="%s" target="_blank">Renew now and enjoy updates</a>, support and Pro templates for another year.', 'elementor-pro' ), $renew_url );
				}
	...
	private function handle_dashboard_admin_widget() {
		add_action( 'elementor/admin/dashboard_overview_widget/after_version', function() {
			...
			echo '<span class="e-overview__version">' . sprintf( esc_html__( 'Elementor Pro v%s', 'elementor-pro' ), ELEMENTOR_PRO_VERSION ) . '</span>';
		} );
		...
		<div class="wrap elementor-admin-page-license">
			...
			<form class="elementor-license-box" method="post" action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>">
				...
				<?php if ( empty( $license_key ) ) : ?>
					...
					<ol>
						<li><?php printf( esc_html__( 'Log in to <a href="%s" target="_blank">your account</a> to get your license key.', 'elementor-pro' ), 'https://go.elementor.com/my-license/' ); ?></li>
						<li><?php printf( esc_html__( 'If you don\'t yet have a license key, <a href="%s" target="_blank">get Elementor Pro now</a>.', 'elementor-pro' ), 'https://go.elementor.com/pro-license/' ); ?></li>
						<li><?php echo esc_html__( 'Copy the license key from your account and paste it below.', 'elementor-pro' ); ?></li>
					</ol>
