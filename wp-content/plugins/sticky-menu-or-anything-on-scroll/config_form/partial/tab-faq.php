<h2 class="smoa-tab-title"><i class="sa-icon sa-faq"></i> FAQ</h2>
<p class="smoa-tab-description">Please read the FAQ below first. If that doesn't help <a href="#support" class="open-beacon">contact our friendly support</a>, they'll respond ASAP!.</p>

<button type="button" class="smoa-collapsible" smoa-collapsible-id="2" smoa-collapsible-type="faq"><?php _e('Q: I need help!', 'sticky-menu-or-anything-on-scroll'); ?></button>
<div class="smoa-content" smoa-content-id="2" smoa-content-type="faq">
    <p>Click the support widget in the lower right corner of this screen.</p>
</div>

<button type="button" class="smoa-collapsible" smoa-collapsible-id="3" smoa-collapsible-type="faq"><?php _e('Q: I selected a class/ID in the settings screen, but the element doesn\'t stick when I scroll down. Why not?', 'sticky-menu-or-anything-on-scroll'); ?></button>
<div class="smoa-content" smoa-content-id="3" smoa-content-type="faq">
    <p><?php _e('First, make sure that if you select the element by its classname, it is preceded by a dot (e.g. ".main-menu"), and if you select it by its ID, that it\'s preceded by a pound/hash/number sign (e.g. "#main-menu").  Also, make sure there is only ONE element on the page with the selector you\'re using. If there is none, or more than one element that matches your selector, nothing will happen.', 'sticky-menu-or-anything-on-scroll'); ?></p>
</div>

<button type="button" class="smoa-collapsible" smoa-collapsible-id="4" smoa-collapsible-type="faq"><?php _e('Q: Once the element becomes sticky, it\'s not positioned/sized properly at all.', 'sticky-menu-or-anything-on-scroll'); ?></button>
<div class="smoa-content" smoa-content-id="4" smoa-content-type="faq">
    <p><?php _e('Due to the nature of CSS, there are situations where an element will not stick properly, usually if it has specific properties that are used to manipulate its location and/or dimensions. If your sticky element has any of the following properties, this could cause conflicts:</p><ul><li>- negative margins</li><li>- absolute positioning</li><li>- top/left/bottom/right properties</li><li>- "display: inline"</li><li>- "!important" applied to any of its properties</li></ul><p>Try to avoid all this where possible, but if you can\'t, using the plugin in Legacy Mode (see below) helps sometimes.<br>Another situation that can cause trouble, is when any parent of your sticky element has the "transform" CSS property applied to it.', 'sticky-menu-or-anything-on-scroll'); ?></p>
</div>

<button type="button" class="smoa-collapsible" smoa-collapsible-id="5" smoa-collapsible-type="faq"><?php _e('Q: Once the element becomes sticky, it\'s not responsive and doesn\'t resize when I change the browser size.', 'sticky-menu-or-anything-on-scroll'); ?></button>
<div class="smoa-content" smoa-content-id="5" smoa-content-type="faq">
    <p><?php _e('This is a known (and annoying) bug in the plugin that I haven\'t been able to solve properly yet. For some sites (when the element does not contain any Javascript interactivity, usually), it sometimes helps to use the plugin in Legacy Mode (see below).', 'sticky-menu-or-anything-on-scroll'); ?></p>
</div>

<button type="button" class="smoa-collapsible" smoa-collapsible-id="6" smoa-collapsible-type="faq"><?php _e('Q: Is it possible to add some styles to the element but <em>only</em> when it\'s sticky?', 'sticky-menu-or-anything-on-scroll'); ?></button>
<div class="smoa-content" smoa-content-id="6" smoa-content-type="faq">
    <p><?php _e('To add styles to your sticky element when it\'s <em>not sticky</em>, use classname ".element-is-not-sticky".<br>To add styles to your sticky element <em>only</em> when it\'s sticky, use classname ".element-is-sticky".', 'sticky-menu-or-anything-on-scroll'); ?></p>
...
<button type="button" class="smoa-collapsible" smoa-collapsible-id="7" smoa-collapsible-type="faq"><?php _e('Q: Once the element becomes sticky, there\'s a brief moment where you see it twice.', 'sticky-menu-or-anything-on-scroll'); ?></button>
<div class="smoa-content" smoa-content-id="7" smoa-content-type="faq">
    <p><?php _e('If you\'re using the plugin in Legacy Mode (see below), this happens when the sticky element (or any of its contents) has a CSS transition applied to it. Because the original element becomes invisible (and a cloned copy of it becomes visible), the visible-to-invisible status change will take place with a transition (ie. not instantly). Either remove any of the transitions the element has, or try disabling the Legacy Mode.', 'sticky-menu-or-anything-on-scroll'); ?></p>
</div>

<button type="button" class="smoa-collapsible" smoa-collapsible-id="10" smoa-collapsible-type="faq"><?php _e('Q: Still doesn\'t work. What could be wrong?', 'sticky-menu-or-anything-on-scroll'); ?></button>
<div class="smoa-content" smoa-content-id="10" smoa-content-type="faq">
    <p><?php _e('Check the "Debug Mode" checkbox in the plugin\'s settings. Reload the page and you may see errors in your browser\'s console window. If you\'ve used a selector that doesn\'t exist, OR there are more of them on the page, you will be notified of that in the console.', 'sticky-menu-or-anything-on-scroll'); ?></p>
</div>

<button type="button" class="smoa-collapsible" smoa-collapsible-id="12" smoa-collapsible-type="faq"><?php _e('Q: What is Legacy Mode? Should I use it?', 'sticky-menu-or-anything-on-scroll'); ?></button>
<div class="smoa-content" smoa-content-id="12" smoa-content-type="faq">
    <p><?php _e('In version 2.0 of the plugin, a new/better method for making elements sticky was introduced. Before this, a sticky element would essentially be a cloned copy of the original element. Now, the element itself becomes sticky and doesn\'t use copying and cloning anymore, which solves a lot of issues.', 'sticky-menu-or-anything-on-scroll'); ?></p>
...
<button type="button" class="smoa-collapsible" smoa-collapsible-id="13" smoa-collapsible-type="faq"><?php _e('Q: What is this Dynamic Mode thing exactly?', 'sticky-menu-or-anything-on-scroll'); ?></button>
<div class="smoa-content" smoa-content-id="13" smoa-content-type="faq">
    <p><?php _e('This mode is only available in Legacy Mode, and to properly explain this, we\'ll need to go a little deeper in the plugin\'s functionality. So bear with me...', 'sticky-menu-or-anything-on-scroll'); ?></p>
